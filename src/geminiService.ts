...
✅ 要求：用細節描寫紮根世界觀，展現主角年齡、成長背景所對應的真實面貌（稚氣、迷茫、對未來的期待或恐懼），在章節末尾僅可埋下一個微小的伏筆，讓讀者對後續產生好奇即可。\n`;
  }
...
3. 【章節結尾守則】：章節結尾必須以「當下場景自然收束」收尾，就像小說一章的正常結束——可以是夜幕降臨後主角休息、一段對話告一段落、短暫的情緒沉澱等。❌ 嚴禁使用「或許隱藏著…」「更大的挑戰即將…」「真相正在等待…」這類前瞻性懸念句作為章末，也不要讓每章都以未完待續的語氣結束。讀者自然會翻到下一章，不需要被提示。
4. 【主角真實年齡與背景守則】：主角的思維模式、反應方式、判斷能力，必須嚴格符合其年齡、成長環境與人生閱歷。
   - 一個剛高中畢業的18歲少年，面對複雜局面的第一反應是「慌亂、本能、憑直覺」，絕對不是「冷靜全盤推演、商業邏輯分析、精準預判對方意圖」。他能做到的，頂多是靠著年輕人的衝勁、某次課堂上學到的片段知識、或對方無意透露的線索，歪打正著地解決問題。
   - 同理：農村出身的孩子不懂官場禮儀；從未習武的人第一次拿劍會手抖；普通家庭長大的學生不熟悉有錢人的社交規則。這些「不知道」本身就是豐富的故事素材。
   - 主角的能力成長必須有「觸發原因」：被人指點、親身失敗、反覆練習、看書自學——每一步成長都要有代價和過程，不能無中生有地「突然學會了」。
   - ❌ 禁止：主角剛出場就展現出與其背景不符的高超技能、老練思維或超齡沉穩。
5. 【配角個性一致性守則】：本章出現的每一個有名字的角色，其言行舉止、說話方式必須完全符合詞彙表中已建立的個性設定。
   - 每個角色的對話語氣必須有明顯差異：老練圓滑的人說話迂迴；直率莽撞的人開口就衝；陰沉多疑的人話少而針鋒相對；溫柔體貼的人措辭輕聲細語。
   - 角色除了說話，還要有「習慣動作」或「小癖好」：緊張時摸耳朵、思考時皺眉、高興時手舞足蹈——這些細節讓角色有血有肉，比對話更令人印象深刻。
   - 角色之間的互動要體現各自的性格衝突或默契，不要每個人都禮貌有加、和顏悅色。
5. 小說標題 (novelTitle)：如果是第一章，請原創一個契合故事大綱的宏大小說名稱；若非第一章，請回傳原小說標題即可。
6. 章節標題 (chapterTitle)：本章的子標題，例如：「第一章：命運的交錯」或「第二章：幽谷深處」。如果是在大結局模式下，標題必須體現這是最終章/結局。
7. 主角狀態更新 (characterUpdate)：
   - 請根據本章發展更新主角的屬性、當前目標和身體狀態。
   - 【道具清單嚴格管理】inventory 只保留主角當下確實持有的物品。本章中用掉、消耗、遺失、贈出、破壞或不再攜帶的道具必須從清單中刪除，不可保留無用項目堆積。
   - 請提供符合故事的 attributes（生命值、精神值、境界或實力、特質，值使用字串）。
8. 名詞/實體增量更新 (glossaryUpdates)：
   - 【只更新有變化的條目】僅提交本章「新出現」或「資訊有重大變動」的條目。對於上一章已存在、本章無新發展的條目，不要重複提交。
   - 【人物條目必填個性】凡是 category 為「人物」的條目，description 中必須包含以下要素：① 外貌特徵（1-2句）② 核心個性與行事風格（例：城府深沉、說話言簡意賅，習慣用沉默施壓）③ 標誌性小習慣或口頭禪（例：總在說話前輕拍桌面；慣用「這便是命」作結語）④ 驅動其行動的核心動機或秘密（例：表面護衛實則暗中監視主角）。這些設定一旦建立，後續章節必須嚴格遵守。
   - 類別 category 只能是以下之一：'人物', '組織', '地點', '道具', '其他'。
9. 名詞移除建議 (glossaryRemovals)：列出本章之後「確定不再出現於故事」的名詞名稱陣列（如：已死亡的次要路人、一次性場景、主角已離開且不會再回去的地點）。若無可移除項目則回傳空陣列。`;
...
export function getResponseSchema() {
  return {
    ...
    properties: {
      ...
      glossaryUpdates: {
        type: "ARRAY",
        description: "本章中新出現，或資訊有重大變動的名詞條目。若上一章已有且本章無新資訊的條目，不要重複提交。",
        items: {
          type: "OBJECT",
          properties: {
      ...
      glossaryRemovals: {
        type: "ARRAY",
        description: "本章之後確定不再出現於故事的名詞名稱列表（已死亡的次要路人、一次性場景地點等）。若無可移除項目則回傳空陣列。",
        items: { type: "STRING" }
      },
      newArcEvent: {
        type: "STRING",
    ...
    required: ["novelTitle", "chapterTitle", "content", "characterUpdate", "glossaryUpdates", "glossaryRemovals"]
  };
}
/**
