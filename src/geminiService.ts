...
4. 小說標題 (novelTitle)：如果是第一章，請原創一個契合故事大綱的宏大小說名稱；若非第一章，請回傳原小說標題即可。
5. 章節標題 (chapterTitle)：本章的子標題，例如：「第一章：命運的交錯」或「第二章：幽谷深處」。如果是在大結局模式下，標題必須體現這是最終章/結局。
6. 主角狀態更新 (characterUpdate)：
...
7. 名詞/實體增量更新 (glossaryUpdates)：
   - 【只更新有變化的條目】僅提交本章「新出現」或「資訊有重大變動」的條目。對於上一章已存在、本章無新發展的條目，不要重複提交。
   - 類別 category 只能是以下之一：'人物', '組織', '地點', '道具', '其他'。
8. 名詞移除建議 (glossaryRemovals)：列出本章之後「確定不再出現於故事」的名詞名稱陣列（如：已死亡的次要路人、一次性場景、主角已離開且不會再回去的地點）。若無可移除項目則回傳空陣列。`;
